...
//u1.id, u1.name, u2.id, u2.name
    @Query(value = "SELECT dialog.id, message, is_read as isRead, date_time as dateTime, u1.id as user1Id, " +
            ...
            , nativeQuery = true)
    Page<DialogView> findDialogsByUserId(@Param("userId") Long userId, Pageable pageable);

    Page<Dialog> findDialogsBySenderIdOrRecipientId(Long sender_id, Long recipient_id, Pageable pageable);
